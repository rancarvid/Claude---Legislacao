"""
⚠️ FICHEIRO DE BACKUP - NÃO USE A NÃO SER POR INSTRUÇÃO EXPLÍCITA

Este ficheiro é uma cópia de segurança de gerar_comparativo_reuniao.py
criada em 2026-03-02 antes de tentativas de reordenação e preenchimento.

QUANDO USAR:
- Apenas se receber a instrução explícita: "reverter para python_backup"
- Se a Opção 2 de reordenação do python_remake.md falhar
- Para comparação com versão atualizada

COMO USAR (se necessário):
  cp python_backup.py gerar_comparativo_reuniao.py
  git restore gerar_comparativo_reuniao.py (OU comando de revert)

Versão original contém:
- 9 artigos: ART-05, 06, 06a, 07, 08, 09, 10, 13, 17
- Campos vazios para ART-11, 12, 14, 15 (não implementados ainda nesta versão)
- Ordem original do array ARTIGOS (ART-17 em primeiro lugar)

---
Gera ficheiro Excel + HTML comparativo para reunião artigo a artigo do @regulamento.
Artigos cobertos: 05-10, 13, 17 (com 11-15 em desenvolvimento).
"""
